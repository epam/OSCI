...
def get_req_param(req: func.HttpRequest, param_name: str, default=None):
...
def parse_date_field(date_field_value: Optional[str] = None) -> Optional[datetime.datetime]:
    return date_field_value \
        if date_field_value is None \
        else datetime.datetime.fromisoformat(date_field_value.replace('Z', ''))
